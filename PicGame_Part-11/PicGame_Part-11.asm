...
INIT              CALL     CLEAR_RAM            ; Clear all RAM values.
                  MOVLW    0x04                 ; Set the initial maximum sequence value for default level 1.
                  MOVWF    SEQ_MAX_COUNT
...
;/*****************************************************/
MODE_GAME         MOVF     SUB_MODE, F          ; Is current sub mode, initialize?
                  BTFSS    STATUS, Z
                  GOTO     SUB_GAME_PLAY        ; Sub mode play.
                  MOVLW    SM_GAME_PLAY         ; Set the next sub mode to play.
                  MOVWF    SUB_MODE
                  MOVLW    LED_SEQ_NULL         ; Don't display LEDs for a period, to indicate game start.
                  CALL     SET_LED_ANIM
                  CLRF     SEQ_COUNT            ; Start game at first LED sequence.
                  BCF      APP_FLAGS, F_PLAYER_TIMEOUT ; Ensure player time out flag is clear at game start.
                  GOTO     SUB_GAME_END
SUB_GAME_PLAY     BTFSC    APP_FLAGS, F_PLAYER_TIMEOUT ; Check for player turn time out.
                  GOTO     SUB_GAME_LOSE
                  MOVFW    SEQ_MAX_COUNT        ; Check for player win, full sequence count.
                  XORWF    SEQ_COUNT, W
                  BTFSC    STATUS, Z            ; Player wins at the end of the sequence.
                  GOTO     SUB_GAME_WIN
                  MOVLW    PAUSE_SHORT          ; Pause for a period before the start of each sequence.
                  CALL     PAUSE
                  CALL     SEQUENCE_ADD         ; Add next sequence LED pattern to end of sequence.
                  CALL     SEQUENCE_PLAY        ; Play LED sequence patterns from start to current sequence.
                  CALL     SEQUENCE_REPEAT      ; Verify user key presses match sequence, player turn.
                  MOVF     PLAYER_TIMEOUT_COUNT, F ; Check for player timeout during player turn.
                  BTFSC    STATUS, Z            ; Player loses after player timeout.
                  GOTO     SUB_GAME_LOSE
                  GOTO     SUB_GAME_END

SUB_GAME_WIN      MOVLW    LED_SEQ_WIN          ; Display win LED animation.
                  CALL     SET_LED_ANIM
                  MOVLW    TUNE_WIN             ; Play win tune.
                  MOVWF    TUNE_PTR
                  GOTO     SUB_GAME_SLEEP

SUB_GAME_LOSE     MOVLW    TUNE_LOSE            ; Play lose tune.
                  MOVWF    TUNE_PTR
                  MOVLW    LOSE_LOOP_COUNT      ; Flash the correct next LED sequence. 
                  MOVWF    TEMP2
LOSE_LOOP         MOVLW    (1 << F_LED_SEQ_PAUSE) ; Flag the displayed LEDs not to animate.
                  MOVWF    LED_STATE            ; Clear LED display.
                  MOVLW    PAUSE_VERY_SHORT     ; Display no LEDs for a very short period of time.
                  CALL     PAUSE
                  MOVFW    LED_COMPARE          ; Display the correct LED sequence.
                  IORWF    LED_STATE, F
                  MOVLW    PAUSE_SHORT          ; Pause for a period to show player correct sequence.
                  CALL     PAUSE
                  DECFSZ   TEMP2
                  GOTO     LOSE_LOOP                  
                  MOVLW    LED_SEQ_LOSE         ; Display lose LED animation.
                  CALL     SET_LED_ANIM

SUB_GAME_SLEEP    MOVLW    PAUSE_LONG           ; Delay before going to sleep for low power after game end.
                  CALL     PAUSE
                  MOVLW    M_SLEEP              ; Change to low power sleep mode.
                  CALL     SET_MODE
SUB_GAME_END      CLRF     KEY_PRESS            ; Clear processed key press flags.
                  RETURN
...
;/**************************************************************************/
SEQUENCE_ADD      CALL     GET_RAND             ; Gat a random value.
                  MOVWF    TEMP2                ; Store the random value.
                  MOVLW    SEQUENCE             ; Point to the next empty LED sequence memory location.
                  ADDWF    SEQ_COUNT, W
                  MOVWF    FSR
                  MOVFW    TEMP2                ; Restore the random value.
                  MOVWF    INDF                 ; Save the random value to the next sequence memory location.
                  INCF     SEQ_COUNT            ; Increment the sequence count for the current sequence.
                  RETURN
...
;/**************************************************************************/
SEQUENCE_PLAY     CLRF     TEMP3                ; Reset sequence pointer.
SEQ_PLAY_NEXT     MOVLW    (1 << F_LED_SEQ_PAUSE) ; Flag the displayed LEDs not to animate.
                  MOVWF    LED_STATE            ; Clear LED display.
                  MOVLW    PAUSE_VERY_SHORT     ; Display no LEDs for a very short period of time
                  CALL     PAUSE                ; between each LED sequence being displayed.
                  MOVLW    SEQUENCE             ; Get next sequence value.
                  ADDWF    TEMP3, W
                  MOVWF    FSR
                  MOVFW    INDF                 ; Read the current sequence value from memory.
                  MOVWF    TEMP4                ; Store current sequence value for manipulation.
                  MOVFW    SIMULTANEOUS         ; Get maximum number of simultaneous LEDs to light.
                  MOVWF    TEMP2                ; Store simultaneous count for a loop counter.
                  INCF     TEMP2                ; Convert from zero offset to minimum value of one.
                  CLRF     TEMP1                ; Construct a beep note from the LED pattern into TEMP1.
SEQ_PLAY_LOOP     MOVLW    b'00000011'          ; Each LED offset definition is in each two bits of the sequence value.
                  ANDWF    TEMP4, W             ; Get the next LED offset value.
                  IORWF    TEMP1, F             ; The beep note is a combination of each LED offset.
                  ADDLW    LED_SEQ_ROT_CLOCK    ; Convert LED offset to an LED IO value.
                  CALL     READ_EEPROM          ; Look up selected level LED value.
                  IORWF    LED_STATE, F         ; Add the defined LED to the displayed LEDs.
                  RRF      TEMP4                ; Move to the next LED offset definition,
                  RRF      TEMP4                ; the next two bits of the current sequence value.
                  DECFSZ   TEMP2                ; Display upto the maximum LEDs for selected simultaneous level.
                  GOTO     SEQ_PLAY_LOOP
                  MOVFW    TEMP1                ; Use the beep note as an offset into the beep frequency table.
                  ADDLW    0x0A                 ; Make beep a higher frequency.
                  IORLW    0x10                 ; Set beep period to short.
                  CALL     START_BEEP           ; Start playing the beep.
                  MOVLW    PAUSE_SHORT          ; Display the current LED sequence for a period of time.
                  CALL     PAUSE
                  MOVLW    SLEEP_TIMEOUT        ; Reset inactive count, to keep awake during the sequence play.
                  MOVWF    SLEEP_TIMEOUT_COUNT
                  INCF     TEMP3                ; Point to next value in sequence.
                  MOVFW    TEMP3                ; Loop until end of sequence.
                  XORWF    SEQ_COUNT, W
                  BTFSS    STATUS, Z
                  GOTO     SEQ_PLAY_NEXT
                  CLRF     LED_STATE            ; Clear LED display after playing the current full LED sequence.
                  RETURN
...
;/* wrong.                                                                */
;/*************************************************************************/
SEQUENCE_REPEAT   CLRF     TEMP3                ; Reset sequence pointer.
SEQ_REPEAT_NEXT   MOVLW    PLAYER_TIMEOUT       ; Reset player turn timeout.
                  MOVWF    PLAYER_TIMEOUT_COUNT
                  MOVLW    SLEEP_TIMEOUT        ; Reset inactive count, to keep awake during the sequence play.
                  MOVWF    SLEEP_TIMEOUT_COUNT
                  CLRF     LED_COMPARE          ; Construct the current LED sequence value to compare against the player.
                  MOVLW    SEQUENCE             ; Get next sequence value.
                  ADDWF    TEMP3, W
                  MOVWF    FSR
                  MOVFW    INDF                 ; Read the current sequence value from memory.
                  ...
                  INCF     TEMP2                ; Convert from zero offset to minimum value of one.
                  CLRF     TEMP1                ; Construct a beep note from the LED pattern into TEMP1.
SEQ_REPEAT_LOOP   MOVLW    b'00000011'          ; An LED offset definition is in each two bits of the sequence value.
                  ANDWF    TEMP4, W             ; Get the next LED offset value.
                  IORWF    TEMP1, F             ; The beep note is a combination of each LED offset.
                  ADDLW    LED_SEQ_ROT_CLOCK    ; Convert LED offset to an LED IO value.
                  CALL     READ_EEPROM          ; Look up selected level LED value.
                  IORWF    LED_COMPARE, F       ; Construct the current LED sequence value to compare against the player.
                  RRF      TEMP4                ; Move to the next LED offset definition.
                  RRF      TEMP4
                  DECFSZ   TEMP2                ; Display upto the maximum LEDs for selected simultaneous level.
                  GOTO     SEQ_REPEAT_LOOP

                  CLRF     KEY_LOG              ; During player turn. Log key presses while any key is held down.
                  CALL     WAIT_KEY_DOWN        ; Wait for the first key to be pressed.
                  CALL     WAIT_KEY_UP          ; Wait for all keys to be released, the key log is the players turn.

                  MOVFW    KEY_LOG              ; If key press does not compare to LED sequence, force lose.
                  ANDLW    (1 << SW_LED_1)|(1 << SW_LED_2)|(1 << SW_LED_3)|(1 << SW_LED_4)
                  XORWF    LED_COMPARE, W
                  BTFSS    STATUS, Z
                  CLRF     PLAYER_TIMEOUT_COUNT ; Force a player time out, to force a lose if compare fails.

                  MOVF     PLAYER_TIMEOUT_COUNT, F ; Test for any player time out and end player turn on a time out.
                  BTFSC    STATUS, Z
                  GOTO     SEQ_REPEAT_END
                  ...
                  XORWF    SEQ_COUNT, W
                  BTFSS    STATUS, Z
                  GOTO     SEQ_REPEAT_NEXT
SEQ_REPEAT_END    RETURN
